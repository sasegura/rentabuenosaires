...
const Adicionar = (props) => {
    ...
    const BotonesAdicionar = () => {
        return (
            <React.Fragment>
                <Button label="New" icon="pi pi-plus" className="p-button-success p-mr-2" onClick={()=>alert("New")} />
                <Button label="Delete" icon="pi pi-trash" className="p-button-danger" onClick={()=>alert("Delete")} />
            </React.Fragment>
        )
    }  
        ...
        return(
            ...
            <Row>
              <Col className="text-center ml-auto mr-auto" lg="6" md="8">
                <InputGroup>
                    <InputText value={nombreDestino} className="borderradius30" onChange={(e) => setNombreDestino(e.target.value)} />
                </InputGroup>
    ...
    async function deletepiso() {
        ...
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Destino Deleted', life: 3000 });
    }

    const pisoDialogFooter = (
        <React.Fragment>
            <Button label="Cancel" icon="pi pi-times" className="p-button-text" onClick={hideDialog} />
            <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={savedestino} />
        </React.Fragment>
    );
    ...
    async function savedestino (){
        ...
        const url = '/destinos';
            try {
                const pisoDatos = await AxiosConexionConfig.patch(url+"/"+destino.iddestino, destino);
            } catch (e) {
                console.log(e);
            }
        setDestinoDialog(false);
    ...
    const getDestino = () => {
        return (
            <div  className="section datatable-responsive-demo">
                ...
                <Dialog visible={destinoDialog} style={{ width: '450px' }} header="Adicionar Piso" modal className="p-fluid" 
                    footer={pisoDialogFooter} onHide={hideDialog}
                >
                    <div className="p-field">
                        <label htmlFor="nombredestino">Nombre</label>
                        <InputText id="nombredestino" value={destino?.nombre} onChange={(e) => onInputChange(e, 'nombre')} required autoFocus className={classNames({ 'p-invalid': submitted && !destino.nombre })} />
                        {submitted && !destino?.nombre && <small className="p-error">Name is required.</small>}
                    </div>          
                </Dialog>
                <Dialog visible={destinoDeleteDialog} style={{ width: '450px' }} header="Confirm" modal footer={deletepisosDialogFooter} onHide={hideDeletepisosDialog}>
                <div className="confirmation-content">
                    <i className="pi pi-exclamation-triangle p-mr-3" style={{ fontSize: '2rem'}} />
                    {destino && <span>Are you sure you want to delete the selected pisos?</span>}
                </div>
            </Dialog>
                {selectedDestino!==null?<AdicionarTabla destino={selectedDestino}/>:<Fragment></Fragment>}
